...
{
    [Route(ConfigEndPoint.Prefix + ConfigEndPoint.Version + "pack-prices")]
    [ApiController]
    public class PackPriceController : ControllerBase
    {
        private readonly IPackPriceService _service;

        public PackPriceController(IPackPriceService service)
        {
            _service = service;
        }
        ...
        public async Task<ActionResult> Insert(PackPriceEntity entity)
        {
            try
            {
        ...
        [HttpPut]
        [Authorize(Roles = "admin")]
        public async Task<ActionResult> Update(Guid id, PackPriceEntity entity)
        {
            try
            {
